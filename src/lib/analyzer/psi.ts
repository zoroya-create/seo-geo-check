...
function buildPSIChecks(mobile: PSIMetrics | null, desktop: PSIMetrics | null): CheckItem[] {
  ...
  // TTFB
  const ttfb = mobile?.ttfb ?? -1;
  checks.push({
    id: "psi_ttfb",
    label: "TTFB（サーバー応答時間）",
    status: ttfb < 0 ? "skip" : ttfb <= 0.8 ? "ok" : ttfb <= 1.8 ? "warn" : "ng",
    detectedValue: ttfb < 0 ? "取得不可" : `${ttfb}秒`,
